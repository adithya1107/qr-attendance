...
import {
  ...
  AlertCircle, CheckCircle, BookOpen, GripVertical,
} from 'lucide-react';
...
const COLOR_PALETTE = [
  { bg: 'bg-blue-50 border-blue-200', text: 'text-blue-700', badge: 'bg-blue-100 text-blue-800', ring: 'ring-blue-400' },
  { bg: 'bg-violet-50 border-violet-200', text: 'text-violet-700', badge: 'bg-violet-100 text-violet-800', ring: 'ring-violet-400' },
  { bg: 'bg-emerald-50 border-emerald-200', text: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-800', ring: 'ring-emerald-400' },
  { bg: 'bg-amber-50 border-amber-200', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-800', ring: 'ring-amber-400' },
  { bg: 'bg-rose-50 border-rose-200', text: 'text-rose-700', badge: 'bg-rose-100 text-rose-800', ring: 'ring-rose-400' },
  { bg: 'bg-cyan-50 border-cyan-200', text: 'text-cyan-700', badge: 'bg-cyan-100 text-cyan-800', ring: 'ring-cyan-400' },
  { bg: 'bg-orange-50 border-orange-200', text: 'text-orange-700', badge: 'bg-orange-100 text-orange-800', ring: 'ring-orange-400' },
  { bg: 'bg-pink-50 border-pink-200', text: 'text-pink-700', badge: 'bg-pink-100 text-pink-800', ring: 'ring-pink-400' },
];
...
const TimetableManager: React.FC<TimetableManagerProps> = ({ teacherData, courses }) => {
  ...
  const validateForm = () => {
    ...
    // Check overlap with existing slots for same course+day
    const conflict = schedules.find(s => {
      if (editingSlot && s.id === editingSlot.id) return false;
      if (s.course_id !== form.course_id) return false;
      if (String(s.day_of_week) !== String(form.day_of_week)) return false;
      // Overlap check
      return form.start_time < s.end_time && form.end_time > s.start_time;
    });
    if (conflict) errors.start_time = `Overlaps with existing slot (${formatTime(conflict.start_time)} – ${formatTime(conflict.end_time)})`;
  ...
  const slotsByDay = React.useMemo(() => {
    ...
    schedules.forEach(s => { if (map[s.day_of_week] !== undefined) map[s.day_of_week].push(s); });
    return map;
  }, [schedules]);

  // Group slots by course for list view
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {Math.floor(totalHours / 60)}h {totalHours % 60 > 0 ? `${totalHours % 60}m` : ''} / week
          </p>
        </div>
      ...
      {courses.length === 0 && (
        <Alert>
          ...
          <AlertDescription>
            You have no active courses. Create a course first before setting up your timetable.
          </AlertDescription>
        </Alert>
      )}
      ...
      {courses.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {courses.map(c => {
            ...
            const count = (slotsByCourse[c.id] || []).length;
            return (
              <span
                key={c.id}
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${color.bg} ${color.text}`}
              >
                <BookOpen className="h-3 w-3" />
                {c.course_code}
                {count > 0 && (
                  <span className={`ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${color.badge}`}>
                    {count}
                  </span>
                )}
              </span>
        ...
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1 mb-6">
              Add recurring weekly class slots for your courses
            </p>
            ...
            <div className="min-w-[600px]">
              ...
              <div className="grid grid-cols-8 border-b">
                ...
                {DAYS_SHORT.map((d, i) => (
                  <div
                    key={d}
                    className={`p-3 text-center text-xs font-semibold border-l ${
                      i === new Date().getDay()
                  ...
                  >
                    ...
                    {slotsByDay[i].length > 0 && (
                      <div className="mt-0.5 text-[10px] opacity-70">
                        {slotsByDay[i].length} class{slotsByDay[i].length > 1 ? 'es' : ''}
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="grid grid-cols-8">
                ...
                {DAYS.map((_, dayIdx) => (
                  <div
                    key={dayIdx}
                    className={`border-l min-h-[140px] p-2 space-y-1.5 ${
                  ...
                  >
                    {slotsByDay[dayIdx].length === 0 ? (
                      <button
                        onClick={() => {
                          setForm({ ...defaultForm, day_of_week: String(dayIdx) });
                          setEditingSlot(null);
                          setFormErrors({});
                          setDialogOpen(true);
                        }}
                      ...
                      slotsByDay[dayIdx]
                        ...
                        .map(slot => {
                          ...
                          return (
                            <div
                              key={slot.id}
                              className={`relative rounded-lg border p-2 text-xs group cursor-default ${color.bg} ${color.text}`}
                            >
      ...
      {viewMode === 'list' && schedules.length > 0 && (
        <div className="space-y-4">
          {courses
            .filter(c => slotsByCourse[c.id]?.length > 0)
            .map(course => {
              const color = getColor(course.id);
              const slots = (slotsByCourse[course.id] || []).sort(
                (a, b) => a.day_of_week - b.day_of_week || a.start_time.localeCompare(b.start_time)
              );
              return (
                <Card key={course.id} className={`border-l-4 ${color.ring.replace('ring', 'border')}`}>
                  <CardHeader className="pb-2 pt-4 px-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          setForm({ ...defaultForm, course_id: course.id });
                          setEditingSlot(null);
                          setFormErrors({});
                          setDialogOpen(true);
                        }}
                  ...
                  <CardContent className="px-4 pb-4">
                    <div className="space-y-2">
                      {slots.map(slot => (
                        <div
                          key={slot.id}
                          className={`flex items-center justify-between p-3 rounded-lg border ${color.bg}`}
                        >
                          <div className="flex items-center gap-4 flex-1">
                            <div className={`text-sm font-semibold min-w-[36px] ${color.text}`}>
          ...
          {courses.filter(c => !slotsByCourse[c.id]?.length).length > 0 && (
            <Card className="border-dashed">
              <CardContent className="py-4 px-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {courses
                    .filter(c => !slotsByCourse[c.id]?.length)
                    .map(c => {
                      const color = getColor(c.id);
                      return (
                        <button
                          key={c.id}
                          onClick={() => {
                            setForm({ ...defaultForm, course_id: c.id });
                            setEditingSlot(null);
                            setFormErrors({});
                            setDialogOpen(true);
                          }}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-opacity hover:opacity-80 ${color.bg} ${color.text}`}
                        >
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              >
                <SelectTrigger className={formErrors.course_id ? 'border-destructive' : ''}>
                ...
                <SelectContent>
                  {courses.map(c => (
                    <SelectItem key={c.id} value={c.id}>
                      ...
                      <span className="text-muted-foreground ml-2 text-xs">{c.course_name}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              ...
              <div className="grid grid-cols-7 gap-1">
                {DAYS_SHORT.map((d, i) => (
                  <button
                    key={d}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, day_of_week: String(i) }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className={formErrors.start_time ? 'border-destructive' : ''}
                />
                {formErrors.start_time && (
                  <p className="text-xs text-destructive">{formErrors.start_time}</p>
                )}
              </div>
              <div className="space-y-1.5">
                ...
                />
                {formErrors.end_time && (
                  <p className="text-xs text-destructive">{formErrors.end_time}</p>
                )}
              </div>
            </div>
            ...
            {form.start_time && form.end_time && form.start_time < form.end_time && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 rounded-lg px-3 py-2">
                <Clock className="h-3.5 w-3.5" />
                Duration: <span className="font-semibold text-foreground">{getDuration(form.start_time, form.end_time)}</span>
              </div>
            )}
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleSave} disabled={saving}>
              {saving ? (
                ...
                <><CheckCircle className="h-4 w-4 mr-2" />Update Slot</>
              ) : (
                <><Plus className="h-4 w-4 mr-2" />Add Slot</>
              )}
            </Button>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={v => { if (!v) setDeleteConfirmId(null); }}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            This removes the recurring slot from your weekly timetable. Past attendance records won't be affected.
          </p>
